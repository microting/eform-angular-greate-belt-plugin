...
def main():
    ...
    if int(get_git_status()) > 0:
        ...
        os.chdir(os.path.join("eFormAPI", "Plugins", "GreateBelt.Pn", "GreateBelt.Pn"))
        packages = [
            'Microting.eForm', 'Microting.eFormApi.BasePn', 'Microting.ItemsPlanningBase', 'Sentry'
        ]
        process_repository('GreateBelt.Pn.csproj', packages, 'eform-angular-greate-belt-plugin')

        os.chdir("..")
        os.chdir("GreateBelt.Pn.Test")
        packages = [
            'Microsoft.NET.Test.Sdk', 'NSubstitute', 'NUnit', 'NUnit3TestAdapter',
            'NUnit.Analyzers', 'Testcontainers', 'Testcontainers.Mariadb'
        ]
        process_repository('GreateBelt.Pn.Test.csproj', packages, 'eform-angular-greate-belt-plugin')
        ...
        if new_number_of_commits > current_number_of_commits:
            ...
            run_command(f"github_changelog_generator -u microting -p eform-angular-greate-belt-plugin -t {os.getenv('CHANGELOG_GITHUB_TOKEN')}", shell=True)
